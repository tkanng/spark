...
class DAGScheduler(
  ...
  private def getOrCreateShuffleMapStage(
    ...
    shuffleIdToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        ...
        // 这里getMissingAncestor函数返回的是一个Stack,计算逻辑第一个Stage,先被访问
  ...
  private def getOrCreateParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
    // 会顺便创建上游的Stage,但是只返回父亲Stages,不包含爷爷Stage.
            ...
            resultStage.activeJob match {
              case Some(job) =>
                if (!job.finished(rt.outputId)) {
                  ...
                  try {
                    // 这里执行resultHandler函数,对于collect函数,就是执行(iter: Iterator[T]) => iter.toArray
                    // resultTask返回的是结果的原始值;ShuffelMapTask返回的是结果的元数据,比如位置等等
