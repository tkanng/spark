...
private[spark] class TaskSchedulerImpl private[scheduler](
  ...
  private def resourceOfferSingleTaskSet(
    ...
    // 按照这里的逻辑,shuffledOffers.size==3,也即只会调度TaskSet中的3个task,第四个task无法被调度??
    // 其中 spark.task.cpus==1.
    // 答:不会.在上述情况中,由于launchedTask为true,所以还会再次进入resourceOfferSingleTaskSet来调度第4个task!!
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    for (taskSet <- sortedTaskSets) {
      ...
      for (currentMaxLocality <- taskSet.myLocalityLevels) {
        // 遍历所有的调度级别
        // TODO-KQ:这里是不是会出现问题.比如:有三个Executor,freeCore分别是3,2,1.现在需要调度一个大小为4的Taskset
        // 按照这里的逻辑,shuffledOffers.size==3,也即只会调度TaskSet中的3个task,第四个task无法被调度??
        // 其中 spark.task.cpus==1.
        // 答:不会.在上述情况中,由于launchedTask为true,所以还会再次进入resourceOfferSingleTaskSet来调度第4个task!!
        // 这里还解决了一个问题:如果TaskSet中的task还没有被完全被调度上，那么就一直会在这for循环里面执行.
        // 如果launchedTaskAtCurrentMaxLocality为true,那么就还会在之前的localityLevel调度;如果为false,那么会直接
        // 将maxLocality设置为下一个localityLevel(ps:这里只设置了maxLocalityLevel,还有机会调度到Process_LOCAL上,
        // 只是提高了MaxLocalityLevel)
        do {
